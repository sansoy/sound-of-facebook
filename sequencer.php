...
$player = 'ogg_html5';
$autostart = isset($_POST['autostart']) ? $_POST['autostart']: true;
$loop = isset($_POST['loop']) ? $_POST['loop'] : false;
$visible = isset($_POST['visible']) ? $_POST['visible'] : true;
...
for ($k=1;$k<=$numb_drums_instruments;$k++){
...
<?php
$halfplusone = $half+1;
for ($k=$halfplusone;$k<=$numb_drums_instruments;$k++){
?>
<tr>
<td class="mediumgrey"><input type="checkbox" name="aktiv<?=$k?>"<?=$aktiv[$k]?' checked="checked"':''?> /></td>
<td class="mediumgrey">
...
if ($play){
	...
	for ($k=1;$k<=$numb_drums_instruments;$k++) if ($aktiv[$k]){		
		$ch = ($k<5)?10:$k;
		$inst = $_POST["inst$k"];
		$n = $_POST["note$k"];
		$v = $_POST["vol$k"];
